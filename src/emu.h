...
bool mem_read(unsigned long address, unsigned char funct3, unsigned long* load_data) {
  *load_data = 0;
  ...
  if (funct3 && 4) {
    *load_data = *load_data >> 8*(8 - byte_size_lookup[funct3 & 3]);
  } else {
...
unsigned long sign_extend(unsigned long data, unsigned char size) {
  ...
  case 2:
    return (0 - (data & (1<<31))) | (data & ((1UL<<32) - 1));
...
int emulate(unsigned char mem[], unsigned long entry_point) {
  ...
  while (1) {
    ...
    imm_u = (0-(instruction & 0x80000000)) | (instruction & (~0xFFF));
    imm_j = (0-((instruction >> 11) & 0x00100000)) | ((instruction >> 11) & 0x00100000) | ((instruction >> 20) & 0x000007FE) | ((instruction >> 9) & 0x00000800) | (instruction & 0x000FF000);
    imm_i = (0-((instruction >> 20) & 0x00000800)) | ((instruction >> 20) & 0x00000FFF);
    imm_b = (0-((instruction >> 19) & 0x00001000)) | ((instruction >> 19) & 0x00001000) | ((instruction >> 20) & 0x000007E0) | ((instruction << 4) & 0x00000800) | ((instruction >> 7) & 0x0000000E);
    imm_s = (0-((instruction >> 20) & 0x00000800)) | ((instruction >> 20) & 0x00000FE0) | ((instruction >> 7) & 0x0000001F);
    ...
    opimm[3] = rs1 < imm_i;
    opimm[4] = rs1 ^ imm_i;
    opimm[5] = (instruction & 0x40000000) ? (((signed) rs1) >> (imm_i && 63)) : (rs1 >> (imm_i && 63));
    opimm[6] = rs1 | imm_i;
    opimm[7] = rs1 & imm_i;
    ...
    op32[3] = 0;
    op32[4] = 0;
    op32[5] = sign_extend((instruction & 0x40000000) ? (((signed int) rs1) >> (rs2 && 31)) : (rs1 >> (rs2 && 31)), 0b010);
    op32[6] = 0;
    op32[7] = 0;
    ...
    opimm32[3] = 0;
    opimm32[4] = 0;
    opimm32[5] = sign_extend((instruction & 0x40000000) ? (((signed int) rs1) >> (imm_i && 31)) : (rs1 >> (imm_i && 31)), 0b010);

    if(((instruction & 127) == 0b0000011) && mem_read(rs1 + imm_i, (instruction >> 12) & 7, &load_data)){
      printf("Error loading address %lx at pc: %lx", rs1 + imm_i, pc);
      break;
    };

    if(((instruction & 127) == 0b0100011) && mem_write(rs1 + imm_i, (instruction >> 12) & 7, rs2)) {
      printf("Error storing address %lx at pc: %lx", rs1 + imm_i, pc);
      break;
    };
    ...
    wb_res[5] = (instruction & 32) ? (pc + imm_u) : (imm_u);
